...
Use `artwork-production-queue.csv` as the active Books 2–30 art queue. It records the release order, assigned batches, required illustration count, approved/QA counts, and the next action for every production title. Book 1 is intentionally excluded while KDP review is active.
